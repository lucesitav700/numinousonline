...
class Admin {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_igd_dismiss_offer_notice', [ $this, 'dismiss_offer_notice' ] );


	}

	public function dismiss_offer_notice() {
		$offer = get_transient( 'igd_offer_notice' );

		if ( false !== $offer ) {
			$offer['dismissed'] = true;
			set_transient( 'igd_offer_notice', $offer, 3 * DAY_IN_SECONDS );
		}
	}

	public function offer_notice() {
		// check if lifetime plan
		$license = igd_fs()->_get_license();

		if ( ! empty( $license ) && empty( $license->expiration ) ) {
			return;
		}
		...
		if ( false === $offer ) {
			...
			$api_url = 'https://softlabbd.com/offers.php?plugin=integrate-google-drive';
			...
			set_transient( 'igd_offer_notice', $offer, DAY_IN_SECONDS );
		}
		...
		igd()->add_notice( 'info igd-offer-notice', $offer_html );
	...
	private
	function add_main_menu_page(
		$title, $submenu_title, $view
	) {
		$page = add_menu_page( __( 'Integrate Google Drive', 'integrate-google-drive' ), __( 'Google Drive', 'integrate-google-drive' ), 'read',
			'integrate-google-drive', $view, IGD_ASSETS . '/images/drive.png', 30 );
	...
	function add_submenu_page(
		$title, $submenu_title, $view, $slug, $priority = 90
	) {
	...
	public
	function render_recommended_plugins_page() {
		include IGD_INCLUDES . '/views/recommended-plugins.php';
	}

	public
	function lost_authorization_notice() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
