...
function igd_get_files_recursive( $file, $current_path = '', &$list = array(
    'folders' => array(),
    'files'   => array(),
    'size'    => 0,
) )
    ...
    if ( !empty($files) ) {
        foreach ( $files as $file ) {
            ...
            if ( $direct_image && ($is_image || $is_audio || $is_video) ) {
                ...
                if ( $is_image ) {
                    printf(
                        '<img class="igd-embed-image" src="%s" alt="%s" width="%s" height="%s" />',
                        esc_url( $url ),
                        esc_attr( $name ),
                        $embed_width,
                        $embed_height
                ...
                printf(
                    '<iframe class="igd-embed" src="%1$s" frameborder="0" scrolling="no" width="%2$s" height="%3$s" allow="autoplay" allowfullscreen="allowfullscreen" %4$s></iframe>',
                    $url,
                    $embed_width,
                    $embed_height,
                    $sandbox_attrs
...
function igd_remove_cache_folders( $folder_id = null )
{
    $cached_folders = get_option( 'igd_cached_folders', [] );
...
function igd_get_all_parent_folders( $file )
{
    ...
    if ( !empty($file['parents']) ) {
        foreach ( $file['parents'] as $parent_id ) {
            ...
            // Assuming `get_file` is a method to get a file's details using its id
...
function igd_get_free_memory_available()
{
    ...
    if ( $memory_limit < 0 ) {
        ...
        if ( defined( 'WP_MEMORY_LIMIT' ) ) {
            ...
            $memory_limit = 1024 * 1024 * 92;
            // Return 92MB if we can't get any reading on memory limits
        }
...
)
{
    ...
    if ( $is_preview || $popout ) {
        ...
        $doc_types = [
            'doc'          => [ 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.google-apps.document' ],
            'sheet'        => [
            'application/vnd.ms-excel',
            'application/vnd.ms-excel.sheet.macroenabled.12',
            'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            ...
            'presentation' => [ 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.slideshow', 'application/vnd.google-apps.presentation' ],
        ...
        foreach ( $doc_types as $doc_type => $types ) {
            ...
            if ( in_array( $type, $types ) ) {
                ...
                if ( $doc_type === 'doc' || $doc_type === 'sheet' || $doc_type === 'presentation' ) {
                    $url = ( $is_editable ? $url : "https://drive.google.com/file/d/{$id}/preview?rm=minimal" );
                }
                break;
            }
        ...
        if ( !$found ) {
            ...
            } elseif ( $direct_image && (strpos( $type, 'image/' ) === 0 || strpos( $type, 'audio/' ) === 0 || strpos( $type, 'video/' ) === 0) ) {
...
function igd_is_public_file( $file )
{
    ...
    if ( isset( $file['permissions'] ) && isset( $file['permissions']['users'] ) && isset( $file['permissions']['users']['anyoneWithLink'] ) ) {
...
function igd_get_thumbnail_url( $file, $size, $iconSize = 64 )
{
    $id = $file['id'];
    $iconLink = $file['iconLink'];
    $thumbnailLink = ( !empty($file['thumbnailLink']) ? $file['thumbnailLink'] : false );
    $accountId = ( !empty($file['accountId']) ? $file['accountId'] : '' );
    $thumb = str_replace( '/16/', "/{$iconSize}/", $iconLink );
    if ( $thumbnailLink ) {
        
        if ( igd_is_public_file( $file ) ) {
            switch ( $size ) {
                case 'icon':
                    $thumb = "https://drive.google.com/thumbnail?id={$id}&sz=w{$iconSize}-h{$iconSize}";
                    break;
                case 'gallery':
            ...
            if ( strpos( $thumbnailLink, 'google.com' ) !== false ) {
                ...
                switch ( $size ) {
                    case 'icon':
                        $thumb = str_replace( '=s220', '=w64-h64', $thumbnailLink );
                        break;
                    case 'gallery':
...
function igd_should_allow( $item, $filters = array() )
{
    extract( $filters );
...
function igd_delete_cache( $folder_ids = array() )
{
    $active_account = Account::instance()->get_active_account();
    ...
    function tutor_get_template( $template = null, $tutor_pro = false )
    {
        ...
        if ( !file_exists( $template_location ) ) {
            ...
            if ( !file_exists( $template_location ) ) {
                ...
                echo  wp_kses( $warning_msg, array(
                    'code' => true,
                ) ) ;
                ?>
				<?php 
...
{
    return !empty($filters['allowExtensions']) && empty($filters['allowAllExtensions']) || !empty($filters['allowExceptExtensions']) && !empty($filters['allowAllExtensions']) || (!empty($filters['nameFilterOptions']) && (!empty($filters['allowNames']) && empty($filters['allowAllNames'])) || !empty($filters['allowExceptNames']) && !empty($filters['allowAllNames'])) || isset( $filters['showFiles'] ) && empty($filters['showFiles']) || isset( $filters['showFiles'] ) && empty($filters['showFolders']) || !empty($filters['isGallery']) || !empty($filters['isMedia']);
}
...
function igd_can_access( $access_right )
{
    ...
    return !empty(array_intersect( $current_user->roles, $access_users )) || in_array( $current_user->ID, $access_users ) || is_multisite() && is_super_admin();
}
...
function igd_contains_tags( $type = '', $template = '' )
{
    ...
    $post_tags = array_merge( $post_tags, $woocommerce_tags );
    // Merge post tags and WooCommerce tags
...
function igd_replace_template_tags( $data, $extra_tag_values = array() )
{
    $name_template = ( !empty($data['name']) ? $data['name'] : '%user_login% (%user_email%)' );
    $date = date( 'Y-m-d' );
    $time = date( 'H:i' );
    $unique_id = uniqid();
    $search = [ '%date%', '%time%', '%unique_id%' ];
    $replace = [ $date, $time, $unique_id ];
    $name = str_replace( $search, $replace, $name_template );
    // Handle form data
    
    if ( !empty($data['form']) ) {
        $form = $data['form'];
        $search = array_merge( $search, [ '%form_title%', '%form_id%', '%entry_id%' ] );
        $replace = array_merge( $replace, [ $form['form_title'], $form['form_id'], $form['entry_id'] ] );
    ...
    if ( !empty($data['file']) ) {
        $file = $data['file'];
        $search = array_merge( $search, [ '%file_name%', '%file_extension%', '%queue_index%' ] );
        $replace = array_merge( $replace, [ $file['file_name'], $file['file_extension'], $file['queue_index'] ] );
        $name = str_replace( $search, $replace, $name );
    }
    ...
    if ( !empty($data['post']) ) {
        ...
        if ( preg_match_all( '/%post_meta_(.*?)%/', $name_template, $matches ) ) {
            foreach ( $matches[1] as $meta_key ) {
                $meta_key_trimmed = trim( $meta_key );
                $meta_value = get_post_meta( $post_id, $meta_key_trimmed, true );
                $name_template = str_replace( '%post_meta_' . $meta_key_trimmed . '%', $meta_value, $name_template );
            }
            $name = $name_template;
        }
    ...
    if ( !empty($data['wc_order']) ) {
        ...
        $search = array_merge( $search, [ '%wc_order_id%', '%wc_order_date%' ] );
        $replace = array_merge( $replace, [ $order_id, $order_date ] );
        $name = str_replace( $search, $replace, $name );
        //Check if %wc_order_meta_{key}% is in the name template
        
        if ( preg_match_all( '/%wc_order_meta_(.*?)%/', $name_template, $matches ) ) {
            foreach ( $matches[1] as $meta_key ) {
                $meta_key_trimmed = trim( $meta_key );
                $meta_value = get_post_meta( $order_id, $meta_key_trimmed, true );
                $name_template = str_replace( '%wc_order_meta_' . $meta_key_trimmed . '%', $meta_value, $name_template );
            }
            $name = $name_template;
        }
    ...
    if ( !empty($data['wc_product']) ) {
        ...
        if ( preg_match_all( '/%wc_product_meta_(.*?)%/', $name_template, $matches ) ) {
            foreach ( $matches[1] as $meta_key ) {
                $meta_key_trimmed = trim( $meta_key );
                $meta_value = get_post_meta( $product_id, $meta_key_trimmed, true );
                $name_template = str_replace( '%wc_product_meta_' . $meta_key_trimmed . '%', $meta_value, $name_template );
            }
            $name = $name_template;
        }
