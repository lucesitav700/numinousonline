...
ko.bindingHandlers.ameDialog = {
    init: function (element, valueAccessor) {
        var dialog = ko.utils.unwrapObservable(valueAccessor());
        var _ = wsAmeLodash;
        var options = dialog.options ? dialog.options : {};
        ...
        var buttons = (typeof options['buttons'] !== 'undefined') ? ko.utils.unwrapObservable(options.buttons) : [];
    ...
    update: function (element, valueAccessor) {
        ...
        var shouldBeOpen = ko.utils.unwrapObservable(dialog.isOpen);
        //Do nothing if the dialog hasn't been initialized yet.
        var $widget = $element.dialog('instance');
        if (!$widget) {
            return;
        }
...
ko.bindingHandlers.ameOpenDialog = {
    init: function (element, valueAccessor) {
        var clickHandler = function (event) {
            var dialogSelector = ko.utils.unwrapObservable(valueAccessor());
            //Do nothing if the dialog hasn't been initialized yet.
            var $widget = jQuery(dialogSelector);
...
ko.bindingHandlers.ameEnableDialogButton = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        var dialogNode = jQuery(element).closest('.ui-dialog');
        if (dialogNode.length < 1) {
            var body_1 = jQuery(element).closest('body');
